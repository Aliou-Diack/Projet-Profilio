...
st.set_page_config(page_title="Portfolio - Aliou Diack", layout="wide")

st.markdown("<h1 style = 'text-align: center;'>Aliou Diack</h1>", unsafe_allow_html=True)
...
selected = option_menu(
    menu_title=None,
    options=["Accueil", "Projets", "CV", "Contact"],
...
if selected == "Accueil":
    ...
    **📍 Top 10 des athlètes les plus médaillés**  
    ...
    Mes stages à l’**ARD** et à la **SODEFITEX** m'ont permis d’appliquer mes compétences sur des données réelles 
    et de comprendre les enjeux opérationnels des organisations.  
    Ces expériences ont renforcé mon objectif : **aider les structures à prendre de meilleures décisions grâce aux données**.

    ### 📚 Formation continue & veille technologique
    Je poursuis une formation continue via des **MOOCs**, des **webinaires** et une **veille active** sur les innovations en data science.  
    Je crois fermement que la science des données doit **servir le bien commun**, notamment dans les **pays en développement**.

    J’ai ainsi travaillé sur des projets liés à :
    - la **pauvreté**
    - l’**éducation**
    - la **santé publique**
    - la **sécurité alimentaire**

    Ce qui m’a **sensibilisé à la dimension sociale** de l’analyse de données.

    ### 🔍 Ce que je recherche
    Je suis actuellement à la recherche d’un **stage de fin d’études** ou d’un **premier emploi**, dans les secteurs :
    ...
    Je suis convaincu que le **digital est un levier majeur de transformation pour l’Afrique**, et je souhaite contribuer à des **projets à fort impact**.

    ### 🌍 Vision et valeurs
    Mon ambition est de devenir un **expert en data science appliquée au développement durable**.  
    Je souhaite **transmettre mes connaissances** pour **renforcer les capacités locales**.

    Je me considère comme un **acteur du changement**, combinant :

    - Expertise technique  
    - Vision éthique  
    - Responsabilité sociale  

    Je m'intéresse également aux **Systèmes d’Information Géographique (SIG)**, convaincu de la puissance des visualisations cartographiques dans la prise de décision.

    ### 👤 Profil personnel
    Je suis reconnu pour être :
    - Calme, réfléchi et travailleur  
    - Méthodique et persévérant  
    - Capable de m’adapter à des environnements à ressources limitées  
    - Attaché à une communication claire et à la vulgarisation scientifique  

    Je parle **couramment français** et je travaille à **améliorer mon anglais professionnel**.

    ### 📬 Disponible pour :
    - Stages de fin d’études  
    - Missions de consultance junior  
    - Projets collaboratifs en data science appliquée  
    - Opportunités dans la formation ou la recherche appliquée  

    Je suis toujours **ouvert aux échanges**, **retours** et **conseils** pour progresser et contribuer utilement à mon environnement.
    """)

    # Configuration de la page
    #st.set_page_config(page_title="CV Aliou Diack", page_icon=":briefcase:", layout="wide")
    ...
    st.markdown("Réalisé par Aliou Diack © 2025")
